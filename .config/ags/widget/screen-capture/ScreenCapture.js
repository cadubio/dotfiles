...
// CheckButton.tsx
var CheckButton = class extends astalify(Gtk4.ToggleButton) {
  static {
...
function OpenScreenCapture() {
  const anchor = Astal7.WindowAnchor.TOP | Astal7.WindowAnchor.BOTTOM | Astal7.WindowAnchor.LEFT | Astal7.WindowAnchor.RIGHT;
  const { CENTER, END } = Gtk4.Align;
  const hypr = Hyprland.get_default();
  const recAudio = /* @__PURE__ */ jsx(MyCheckButton, {});
  function hide() {
    app_default.quit();
  }
  function onKeyPress(_, event) {
    if (event.get_keyval()[1] === Gdk.KEY_Escape) {
      hide();
    }
  }
  function isActive(monitor, audio) {
    if (audio) {
      execAsync([
        "bash",
        "-c",
        `wf-recorder --audio --file=$HOME/gravacoes/captura-\`date +%Y-%m-%d_%H-%M-%S\`.mp4 --codec h264_vaapi -d /dev/dri/renderD128 --bframes 2 --output ${monitor}`
      ]).then((out) => console.log(out)).catch((err) => console.log(err));
    } else {
      execAsync([
        "bash",
        "-c",
        `wf-recorder --file=$HOME/gravacoes/captura-\`date +%Y-%m-%d_%H-%M-%S\`.mp4 --codec h264_vaapi -d /dev/dri/renderD128 --bframes 2 --output ${monitor}`
      ]).then((out) => console.log(out)).catch((err) => console.log(err));
    }
    app_default.toggle_window("openScreenCapture");
    app_default.toggle_window("closeScreenCapture");
  }
  return /* @__PURE__ */ jsx(
    "window",
    {
      ...
      children: /* @__PURE__ */ jsxs("box", { halign: CENTER, valign: CENTER, vertical: true, children: [
        /* @__PURE__ */ jsx("box", { halign: CENTER, children: /* @__PURE__ */ jsx("label", { label: "Selecione o monitor" }) }),
        /* @__PURE__ */ jsx("box", { halign: END, children: recAudio }),
        /* @__PURE__ */ jsx("box", { children: bind(hypr, "monitors").as(
          (ms) => ms.map((monitor) => /* @__PURE__ */ jsx("button", { onClicked: () => isActive(`${monitor.name}`, recAudio.active), children: /* @__PURE__ */ jsx("label", { label: `${monitor.name}` }) }))
        ) })
      ] })
    }
  );
...
// CloseScreenCapture.tsx
function CloseScreenCapture() {
  const anchor = Astal7.WindowAnchor.BOTTOM | Astal7.WindowAnchor.LEFT;
  const { CENTER, END } = Gtk4.Align;
  function hide() {
    app_default.quit();
  }
  ...
  return /* @__PURE__ */ jsx(
    "window",
    {
      ...
      keymode: Astal7.Keymode.ON_DEMAND,
      onKeyReleaseEvent: onKeyPress,
      children: /* @__PURE__ */ jsx("box", { halign: CENTER, valign: CENTER, vertical: true, className: "close-capture", children: /* @__PURE__ */ jsx("button", { onClicked: stopCapture, children: /* @__PURE__ */ jsx("icon", { icon: "media-playback-pause" }) }) })
    }
  );
}
