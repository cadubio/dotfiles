...
Diz-que a classe `Cachorro` extende/é `Animal`. Além de herdar podemos obrigar quem herda a implementar ou sobscrever alguns cmportamentos, neste caso a é obrigatório que a classe seja uma abstração, atraves da palavra reservada `abstract`.
