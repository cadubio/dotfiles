...
> NOTA 2:
> O método `.forEach` só pode ser usado em uma requisação *assíncrona*. Para requisões *síncronas* use o `for in`.
...
```
Alguns métodos de `Map`

`.putIfAbsent`: adiciona um item ao `Map` caso a chave não exista.
...
// Se a chave não existe pode-se usar o ifAbsent
